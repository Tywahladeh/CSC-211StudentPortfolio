...
else if(carouselType[0]=='company'){finalString="&minRating="+userVaribles.options.reviews.min_review_percent;for(var x=1;x<carouselType.length;x++){if(carouselType[x]=="product"){var skus=userVaribles.sku;skus=skus.replaceAll("%3B",";");finalString+="&include_product_reviews=1&sku="+skus;}
...
var skipReview=false;if(review!=undefined){if(review.rating<userVaribles.options.reviews.min_review_percent){skipReview=true;}
if(thirdPartyTesting!=review.third_party){skipReview=true;}
